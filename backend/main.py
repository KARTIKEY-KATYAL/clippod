...
image = (modal.Image.from_registry(
    ...
    .run_commands([
        ...
        "fc-cache -f -v",
        # Pre-download the S3FD model to avoid runtime download issues
        "mkdir -p /asd/model/faceDetector/s3fd",
        "pip install gdown",
        "gdown --id 1KafnHz7ccT-3IyddBsL5yi2xGtxAKypt -O /asd/model/faceDetector/s3fd/sfd_face.pth || echo 'Model download failed, will retry at runtime'"
    ])
    .add_local_dir("asd", "/asd", copy=True))

app = modal.App("clippod", image=image)

volume = modal.Volume.from_name(
    "clippod-model-cache", create_if_missing=True
)
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    if result.returncode != 0:
        print(f"Columbia script failed with return code {result.returncode}")
        print(f"STDOUT: {result.stdout}")
        print(f"STDERR: {result.stderr}")
        # Don't raise exception, just skip this clip
        return
    ...
    if not tracks_path.exists() or not scores_path.exists():
        print(f"Warning: Tracks or scores not found for clip {clip_index}, skipping...")
        return  # Skip this clip instead of raising an error
    ...
    s3_client.upload_file(
        subtitle_output_path, "clippod", output_s3_key)


@app.cls(gpu="L40S", timeout=900, retries=0, scaledown_window=20, secrets=[modal.Secret.from_name("clippod-secret")], volumes={mount_path: volume})
class AiPodcastClipper:
    ...
    def identify_moments(self, transcript: dict):
        ...
        max_retries = 3
        base_delay = 10  # Start with 10 seconds as suggested by the error
        
        # Limit transcript size to avoid hitting token limits
        transcript_str = str(transcript)
        if len(transcript_str) > 50000:  # Adjust this limit as needed
            transcript_str = transcript_str[:50000] + "...]"
            print("Transcript truncated due to length")
...
The transcript is as follows:\n\n""" + transcript_str)
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        print(f"Rate limit exceeded after {max_retries} attempts. Returning empty list.")
                        return "[]"
                else:
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("clippod", s3_key, str(video_path))
